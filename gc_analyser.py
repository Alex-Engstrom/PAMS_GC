# -*- coding: utf-8 -*-
"""
Created on Fri Aug 22 15:18:40 2025

@author: aengstrom, copied from https://github.com/GerardOtiniano/chromatoPy/blob/main/src/chromatopy/FID/FID_Integration_functions.py
"""
...
def baseline(x, y, deg=5, max_it=1000, tol=1e-4):
    ...
    if norm_coeffs == 0:
        coeffs = np.ones(order) * 1e-10
        norm_coeffs = np.linalg.norm(coeffs)
    
    cond = math.pow(max(abs(y).max(), 1e-10), 1.0 / max(order, 1))
    x_vals = np.linspace(0.0, max(cond, 1e-10), len(y))
    base = y.copy()
    
    try:
        vander = np.vander(x_vals, order)
        ...
        for iteration in range(max_it):
            ...
            # Safe division check
            norm_diff = np.linalg.norm(coeffs_new - coeffs)
            norm_old = np.linalg.norm(coeffs)
            if norm_old < 1e-10:
                break
        ...
        # Calculate maximum peak amplitude with safe fallback
        try:
            baseline_fitter = Baseline(x_vals)
            fit, params_mask = baseline_fitter.std_distribution(y, 45)
            mask = params_mask['mask']
            if len(mask) > 10:  # Need sufficient points for std calculation
                min_peak_amp = (np.std(y[mask])) * 2 * 3
            else:
                min_peak_amp = 0.1
        except:
            min_peak_amp = 0.1
        ...
        print(f"Baseline fitting error: {e}")
        # Return sensible fallback values
        return np.zeros_like(original_y), 0.1
...
def run_peak_integrator(data, key, gi, pk_sns, smoothing_params, max_peaks_for_neighborhood, fp, gaussian_fit_mode):
    ...
    if y_max > 0:
        ...
        plt.ylim(0, 1)  # Fallback for flat signals
